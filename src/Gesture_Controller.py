...
class GestureController:
    ...
    def classify_hands(results):
        ...
        if not results or not results.multi_handedness or not results.multi_hand_landmarks:
            return
        ...
        for idx, hand_info in enumerate(results.multi_handedness):
            if idx >= len(results.multi_hand_landmarks):
                break
            try:
                handedness_dict = MessageToDict(hand_info)
    ...
    def start(self):
        ...
        if not GestureController.cap or not GestureController.cap.isOpened():
            print("[GestureController] Camera not opened. Exiting gesture controller.")
            GestureController.gc_mode = 0
            return
        ...
        with mp_hands.Hands(max_num_hands = 2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                if key == 13 or cv2.getWindowProperty('Gesture Controller', cv2.WND_PROP_VISIBLE) < 1:
                    break
